...
export function getAuthClient() {
  const creds = process.env.VITE_GOOGLE_CREDENTIALS_BASE || process.env.GOOGLE_CREDENTIALS_BASE64;
  if (!creds) {
    return null;
  }
...
export const SPREADSHEET_ID = extractSpreadsheetId(process.env.VITE_GOOGLE_SPREADSHEET_ID || process.env.GOOGLE_SPREADSHEET_ID);
  ...
  try {
    ...
    if (!configSheetExists) {
      ...
      await sheets.spreadsheets.values.append({
        ...
        requestBody: {
          values: [['mensagem_topo', 'Bento está internado com diagnóstico de alcapa e estamos orando por ele e pelos seus pais.']],
        },
      });
    }
  } catch (error) {
